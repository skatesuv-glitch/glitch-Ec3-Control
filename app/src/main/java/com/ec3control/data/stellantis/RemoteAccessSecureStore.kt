...
internal data class StoredRemoteAccessSession(
 val accessToken:String,
 val refreshToken:String?,
 val expiresAtEpochMillis:Long
)

internal class RemoteAccessSecureStore(private val context:Context){
 private val prefs=context.getSharedPreferences("ec3_secure_remote_access",Context.MODE_PRIVATE)
 private val alias="ec3_remote_access_v1"
 ...
 fun save(accessToken:String,refreshToken:String?,expiresAtEpochMillis:Long){
  require(accessToken.isNotBlank())
  val plain=buildJsonObject{
   put("access",accessToken);refreshToken?.let{put("refresh",it)};put("expires",expiresAtEpochMillis)
 ...
 fun load():StoredRemoteAccessSession?=try{
  ...
  StoredRemoteAccessSession(
   requireNotNull(o["access"]?.jsonPrimitive?.content),
   o["refresh"]?.jsonPrimitive?.content,
   requireNotNull(o["expires"]?.jsonPrimitive?.content).toLong()
  )
